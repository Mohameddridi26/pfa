        ...
        <div class="time-display">
          ...
          <div class="time-content">
            <span class="time-value" [class.time-warning]="order.isUrgent">
              {{ getPreparationTime() }}<span class="time-unit">min</span>
            </span>
            <span class="estimated-time" *ngIf="order.estimatedTime && getElapsedMinutes() !== order.estimatedTime">
      ...
      <div class="order-items">
        ...
        <div *ngFor="let item of order.items; trackBy: trackByItemId" 
          ...
          <div class="item-details">
            ...
            <span class="item-category" [class]="'category-' + (getStaticCategory(item.name) ? getStaticCategory(item.name).toLowerCase() : 'inconnue')">
              {{ getCategoryLabel(getStaticCategory(item.name)) }}
            </span>
          </div>
        </div>
      </div>
...
export class OrderCardComponent {
  ...
  ngOnInit() {
    // Debug log to check the order data
    console.log('Order data:', this.order);
    // Log each item's type to diagnose
    this.order.items.forEach(item => {
      console.log(`Item: ${item.name}, Type: ${item.type || 'undefined'}, Static Type: ${this.getStaticCategory(item.name) || 'undefined'}`);
    });
  }
  ...
  getStaticCategory(name: string): string {
    // Static mapping of item names to types
    ...
    return categoryMap[name] || 'INCONNUE'; // Default to INCONNUE if name not found
  }

  getProgressPercentage(): number {
    if (this.order.estimatedTime === 0) return 0;
    return Math.min((this.getElapsedMinutes() / this.order.estimatedTime) * 100, 100);
  }
  ...
  getElapsedMinutes(): number {
    if (!this.order.createdAt) return 0;
  ...
  getPreparationTime(): number {
    if (this.order.estimatedTime && this.order.estimatedTime > 0) {
      return this.order.estimatedTime;
    }
